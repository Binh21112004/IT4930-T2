...
MAX_RETRIES = 3
SAVE_EVERY = 20
PROCESSED_IDS_PATH = 'data/bachhoa-tiki/result/processed_ids.txt'
...
df_id = pd.read_csv('data/bachhoa-tiki/result/product_id_ncds.csv')
...
for idx, pid in enumerate(tqdm(p_ids, total=len(p_ids)), start=1):
    ...
    if idx % SAVE_EVERY == 0:
        ...
        df_comment.to_csv('data/bachhoa-tiki/result/comments_data_ncds.csv', index=False)
