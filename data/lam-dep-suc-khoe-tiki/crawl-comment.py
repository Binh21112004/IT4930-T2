...
headers = {
    ...
    'Referer': 'https://tiki.vn/mot-thoang-ta-ruc-ro-o-nhan-gian-p147920903.html?itm_campaign=CTP_YPD_TKA_PLA_UNK_ALL_UNK_UNK_UNK_UNK_X.168450_Y.1750777_Z.3324609_CN.Mot-Thoang-Ta-Ruc-Ro-O-Nhan-Gian-%28119350vnd%29-%26-%2A&itm_medium=CPC&itm_source=tiki-ads&spid=153563912',
...
df_id = pd.read_csv('data/lam-dep-suc-khoe-tiki/result/product_id_ncds.csv')
...
for pid in tqdm(p_ids, total=len(p_ids)):
    ...
    for i in range(1, 11):
        ...
        params['page'] = i
        try:
            response = requests.get('https://tiki.vn/api/v2/reviews', headers=headers, params=params, cookies=cookies, timeout=20)
        except Exception as e:
            print(f'Error fetching page {i} for product {pid}: {e}')
            time.sleep(random.uniform(3, 6))
            continue
        if response.status_code == 200:
            data = response.json().get('data') or []
            if not data:
                print(f'No comments on product {pid} page {i}, stopping page loop.')
                break
            print('Crawl comment page {} success!!!'.format(i))
            for comment in data:
                rating = comment.get('rating')
                if rating is not None and rating <= TARGET_RATING:
                    result.append(comment_parser(comment))
                    if len(result) >= MAX_COMMENTS:
                        break
        else:
            print('Warning: page {} status {}'.format(i, response.status_code))
        time.sleep(random.uniform(2, 5))
    if len(result) >= MAX_COMMENTS:
        break

df_comment = pd.DataFrame(result)
df_comment.to_csv('data/lam-dep-suc-khoe-tiki/result/comments_data_ncds.csv', index=False)
print(f'Saved {len(result)} comments to data/lam-dep-suc-khoe-tiki/result/comments_data_ncds.csv')